...
void usertrap(void) {
  ...
  if (r_scause() == 8) {
    ...
    // page fault
    uint64 va_fault_page = r_stval();
    uint64 ka = (uint64)kalloc();
    // 禁止访问guard page
    if (ka == 0) {
      p->killed = 1;
    } else if (va_fault_page >= p->sz || va_fault_page <= PGROUNDDOWN(p->trapframe->sp)) {
      kfree((void *)ka);
      p->killed = 1;
    } else {
      memset((void *)ka, 0, PGSIZE);
      if (mappages(p->pagetable, PGROUNDDOWN(va_fault_page), PGSIZE, (uint64)ka,
